 ...
 * Diagnosis → numbered Action Plan → Reversal Guide → Downstream Impact,
 * plus NAVIGATE/SPOTLIGHT chips and Socratic follow-ups.
 */
final class OperationsInstructorFormatter {
    ...
    static String ensureInstructorShape(
        ...
        if (body.contains("**Diagnosis**") || body.contains("↺ Reversal Guide")) {
            return body;
        }
    ...
    private static String reversalGuide(String q, boolean pickerOnly, boolean b2b) {
        ...
        if (pickerOnly) {
            return "Do not delete scans. If a pick was wrong, use Skip & Flag or ask a manager to post an "
                    + "ERROR_CORRECTION ledger reversal — the inventory ledger is append-only.";
        }
        if (q.contains("allocat")) {
            return "On Sales Orders, use **Un-allocate** before the wave ships. That releases reservations "
                    + "without deleting ledger history. After ship, reverse via a return/RMA path.";
        }
        if (q.contains("ledger") || q.contains("reverse") || q.contains("undo") || q.contains("correction")) {
            return "Call the compensating reverse on the mistaken row (`/api/v1/inventory/ledger/{id}/reverse`) "
                    + "or post ERROR_CORRECTION / OFFLINE_CONFLICT_OVERRIDE. Never delete `inventory_ledger` rows.";
        }
        if (q.contains("conflict") || q.contains("409") || q.contains("offline")) {
            return "In the Conflict Panel, choose **Discard** to drop the parked mutation, or **Approve & Re-process** "
                    + "after fixing stock — both leave an audit trail; nothing is silently erased.";
        }
        return "Prefer compensating actions on this page (Cancel, Un-allocate, Reverse, Discard). "
                + "Never delete append-only inventory ledger history.";
    }

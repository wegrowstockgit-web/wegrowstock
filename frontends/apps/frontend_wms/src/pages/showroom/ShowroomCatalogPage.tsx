...
import { fetchPublicShowroomCatalog, mapPortalCatalog, type PortalCatalogItemRaw } from '@/api/portal';
...
import { rolesInclude, useIsAuthenticated, useSessionRoles } from '@/stores/session';
...
export function ShowroomCatalogPage() {
  ...
  const showWholesale = authenticated && rolesInclude(sessionRoles, 'B2B_CUSTOMER');

  const { data: items = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['showroom', 'catalog', showWholesale ? 'b2b' : 'guest', search, category],
    queryFn: async () => {
      if (!showWholesale) {
        return fetchPublicShowroomCatalog();
      }
      const params = new URLSearchParams();
      if (search) params.set('q', search);
      if (category !== 'ALL') params.set('category', category);
      const res = await apiClient.get<PortalCatalogItemRaw[]>(`/api/v1/portal/catalog?${params}`);
  ...
  return (
    <div>
      {!showWholesale && (
        <div
          className="mb-6 rounded-lg border border-accent/30 bg-accent-muted px-4 py-3 text-sm text-text"
          data-testid="showroom-gated-banner"
        >
          Log in or Apply for a Wholesale Account to unlock B2B pricing.{' '}
          <Link to="/showroom/login" className="font-semibold text-accent hover:underline">
            Log in
          </Link>
          {' · '}
          <Link to="/showroom/apply" className="font-semibold text-accent hover:underline">
            Apply
          </Link>
        </div>
      )}
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-text-muted">
          {showWholesale ? 'Tier pricing applied automatically' : 'Guest browsing — wholesale prices hidden'}
        </p>
      </div>
          ...
          {visibleItems.map((item) => {
            ...
            return (
              <Card key={item.id}>
                ...
                {item.category && <p className="mt-1 text-xs text-text-muted">{item.category}</p>}
                {showWholesale ? (
                  ...
                  <p className="mt-3 text-sm font-medium text-text-muted" data-testid="showroom-price-gated">
                    Login to see wholesale price
                  </p>
                )}
                {showWholesale && item.atp != null && (

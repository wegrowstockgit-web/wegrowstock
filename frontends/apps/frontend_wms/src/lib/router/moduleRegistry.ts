...
export type AppModule = {
  ...
  enabled: boolean;
  /**
   * Optional commercial entitlement key (backend AppModule enum name).
   * When set, runtime session entitlements can hide routes/nav after hydration.
   */
...
export function getEnabledModules(entitlements?: readonly string[]): AppModule[] {
  return registeredModules.filter((m) => {
    if (!m.enabled) return false;
    if (!m.commercialModule) return true;
    // Until /me hydrates entitlements, keep build-time enabled modules visible.
    if (!entitlements || entitlements.length === 0) return true;
    return entitlements.includes(m.commercialModule);
  });
}
...
export function getDisabledNavPaths(entitlements?: readonly string[]): Set<string> {
  const enabledIds = new Set(getEnabledModules(entitlements).map((m) => m.id));
...
export function isNavPathEnabled(to: string, entitlements?: readonly string[]): boolean {
  return !getDisabledNavPaths(entitlements).has(to);
}

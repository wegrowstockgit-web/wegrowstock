export type PosLanguage = 'en' | 'es' | 'fr';

const DICTIONARY = {
  en: {
    'app.back': 'Back to register',
    'login.title': 'Retail POS sign-in',
    'login.subtitle': 'Same tenant credentials as the WMS. The register stays usable offline.',
    'login.email': 'Email',
    'login.password': 'Password',
    'login.submit': 'Sign In',
    'login.error': 'Invalid email or password',
    'login.failed': 'Login failed',
    'login.posDenied': 'This account cannot open the retail register',
    'pin.title': 'Enter shift PIN',
    'pin.subtitle': 'The register stays locked until the cashier PIN is verified.',
    'pin.error': 'Invalid shift PIN',
    'register.scanLabel': 'Barcode / UPC',
    'register.scanPlaceholder': 'Scan barcode / UPC',
    'register.add': 'Add',
    'register.item': 'Item',
    'register.upc': 'UPC',
    'register.unitPrice': 'Unit price',
    'register.qty': 'Qty',
    'register.lineTotal': 'Line total',
    'register.empty': 'Scan a barcode to start the sale',
    'register.checkout': 'Checkout',
    'register.subtotal': 'Subtotal',
    'register.grandTotal': 'Grand total',
    'register.taxUs': 'USA sales tax (8.25%)',
    'register.taxMx': 'Mexico IVA (16%)',
    'register.taxUsShort': 'USA',
    'register.taxMxShort': 'Mexico',
    'register.exactCash': 'Exact cash',
    'register.nextDollar': 'Next dollar amount',
    'register.addCustomer': '+ Add Customer',
    'register.customerSearch': 'Search customers',
    'register.noCustomers': 'No matching customers',
    'register.clearCustomer': 'Remove customer',
    'register.solicitarFactura': 'Solicitar Factura',
    'register.facturaTitle': 'Datos fiscales (CFDI)',
    'register.facturaRfc': 'RFC',
    'register.facturaUso': 'Uso de CFDI',
    'register.facturaSave': 'Save invoice data',
    'register.facturaCancel': 'Cancel',
    'register.card': 'Card / terminal',
    'register.backspace': 'Backspace',
    'register.success': 'SUCCESS - NEXT CUSTOMER',
    'register.unknownUpc': 'Unknown UPC {upc}',
    'register.unknownUpcOffline': 'Unknown UPC (Offline) {upc}',
    'register.scanFirst': 'Scan an item first',
    'register.decrease': 'Decrease {name}',
    'register.increase': 'Increase {name}',
    'register.remove': 'Remove {name}',
    'register.removeShort': 'Remove',
    'register.voidTransaction': 'Void transaction',
    'register.voidTitle': 'Void this transaction?',
    'register.voidBody': 'A manager PIN is required. This exception is written to the offline audit trail.',
    'register.voidConfirm': 'Yes, Void Transaction',
    'register.voidCancel': 'Keep sale',
    'register.voidPin': 'Manager PIN',
    'register.voidPinHint': 'Enter the 4-digit override PIN',
    'register.voidInvalidPin': 'Invalid manager PIN',
    'register.voidValue': 'Cart {amount}',
    'register.signIn': 'Sign in',
    'register.offline': 'Offline',
    'register.online': 'Ready',
    'register.placeHint': 'Opened in {place}',
    'register.currencyMismatch': 'WMS {wms} · local {place}',
    'register.itemsCount': '{n} items',
    'register.tendered': 'Tendered',
    'register.changeDue': 'Change due',
    'register.qtyEach': '{qty} × {price}',
    'register.lane': 'Lane',
    'locked.title': 'Retail POS is not enabled',
    'locked.body':
      'This workspace or subscription tier does not include Retail POS. Ask an owner to enable the module in WMS before taking sales.',
    'locked.signin': 'Sign in with another account',
  },
  es: {
    'app.back': 'Volver a la caja',
    'login.title': 'Acceso al POS',
    'login.subtitle': 'Las mismas credenciales del WMS. La caja sigue funcionando sin conexión.',
    'login.email': 'Correo',
    'login.password': 'Contraseña',
    'login.submit': 'Iniciar sesión',
    'login.error': 'Correo o contraseña no válidos',
    'login.failed': 'Error de acceso',
    'login.posDenied': 'Esta cuenta no puede abrir la caja',
    'pin.title': 'Ingresa el PIN de turno',
    'pin.subtitle': 'La caja permanece bloqueada hasta verificar el PIN del cajero.',
    'pin.error': 'PIN de turno no válido',
    'register.scanLabel': 'Código de barras / UPC',
    'register.scanPlaceholder': 'Escanear código / UPC',
    'register.add': 'Añadir',
    'register.item': 'Artículo',
    'register.upc': 'UPC',
    'register.unitPrice': 'Precio',
    'register.qty': 'Cant.',
    'register.lineTotal': 'Importe',
    'register.empty': 'Escanea un código para iniciar la venta',
    'register.checkout': 'Cobro',
    'register.subtotal': 'Subtotal',
    'register.grandTotal': 'Total',
    'register.taxUs': 'Impuesto USA (8.25%)',
    'register.taxMx': 'IVA México (16%)',
    'register.taxUsShort': 'USA',
    'register.taxMxShort': 'México',
    'register.exactCash': 'Efectivo exacto',
    'register.nextDollar': 'Siguiente peso entero',
    'register.addCustomer': '+ Agregar cliente',
    'register.customerSearch': 'Buscar clientes',
    'register.noCustomers': 'Sin clientes coincidentes',
    'register.clearCustomer': 'Quitar cliente',
    'register.solicitarFactura': 'Solicitar Factura',
    'register.facturaTitle': 'Datos fiscales (CFDI)',
    'register.facturaRfc': 'RFC',
    'register.facturaUso': 'Uso de CFDI',
    'register.facturaSave': 'Guardar datos fiscales',
    'register.facturaCancel': 'Cancelar',
    'register.card': 'Tarjeta / terminal',
    'register.backspace': 'Borrar',
    'register.success': 'ÉXITO — SIGUIENTE CLIENTE',
    'register.unknownUpc': 'UPC desconocido {upc}',
    'register.unknownUpcOffline': 'UPC desconocido (sin conexión) {upc}',
    'register.scanFirst': 'Escanea un artículo primero',
    'register.decrease': 'Quitar {name}',
    'register.increase': 'Añadir {name}',
    'register.remove': 'Quitar {name}',
    'register.removeShort': 'Quitar',
    'register.voidTransaction': 'Anular transacción',
    'register.voidTitle': '¿Anular esta transacción?',
    'register.voidBody': 'Se requiere el PIN del gerente. Esta excepción se guarda en la bitácora local.',
    'register.voidConfirm': 'Sí, anular transacción',
    'register.voidCancel': 'Conservar venta',
    'register.voidPin': 'PIN de gerente',
    'register.voidPinHint': 'Ingresa el PIN de 4 dígitos',
    'register.voidInvalidPin': 'PIN de gerente no válido',
    'register.voidValue': 'Carrito {amount}',
    'register.signIn': 'Entrar',
    'register.offline': 'Sin conexión',
    'register.online': 'Listo',
    'register.placeHint': 'Abierto en {place}',
    'register.currencyMismatch': 'WMS {wms} · local {place}',
    'register.itemsCount': '{n} artículos',
    'register.tendered': 'Entregado',
    'register.changeDue': 'Cambio',
    'register.qtyEach': '{qty} × {price}',
    'register.lane': 'Caja',
    'locked.title': 'El POS no está activado',
    'locked.body':
      'Este espacio o plan no incluye Retail POS. Pide a un propietario que active el módulo en el WMS antes de cobrar.',
    'locked.signin': 'Entrar con otra cuenta',
  },
  fr: {
    'app.back': 'Retour à la caisse',
    'login.title': 'Connexion POS',
    'login.subtitle': 'Les mêmes identifiants que le WMS. La caisse reste utilisable hors ligne.',
    'login.email': 'E-mail',
    'login.password': 'Mot de passe',
    'login.submit': 'Connexion',
    'login.error': 'E-mail ou mot de passe invalide',
    'login.failed': 'Échec de connexion',
    'login.posDenied': 'Ce compte ne peut pas ouvrir la caisse',
    'pin.title': 'Saisir le PIN de caisse',
    'pin.subtitle': 'La caisse reste verrouillée jusqu’à la vérification du PIN caissier.',
    'pin.error': 'PIN de caisse invalide',
    'register.scanLabel': 'Code-barres / UPC',
    'register.scanPlaceholder': 'Scanner le code / UPC',
    'register.add': 'Ajouter',
    'register.item': 'Article',
    'register.upc': 'UPC',
    'register.unitPrice': 'Prix',
    'register.qty': 'Qté',
    'register.lineTotal': 'Ligne',
    'register.empty': 'Scannez un code-barres pour commencer la vente',
    'register.checkout': 'Encaissement',
    'register.subtotal': 'Sous-total',
    'register.grandTotal': 'Total',
    'register.taxUs': 'Taxe USA (8,25 %)',
    'register.taxMx': 'TVA Mexique (16 %)',
    'register.taxUsShort': 'USA',
    'register.taxMxShort': 'Mexique',
    'register.exactCash': 'Espèces exactes',
    'register.nextDollar': 'Dollar supérieur',
    'register.addCustomer': '+ Ajouter un client',
    'register.customerSearch': 'Rechercher des clients',
    'register.noCustomers': 'Aucun client correspondant',
    'register.clearCustomer': 'Retirer le client',
    'register.solicitarFactura': 'Solicitar Factura',
    'register.facturaTitle': 'Données fiscales (CFDI)',
    'register.facturaRfc': 'RFC',
    'register.facturaUso': 'Uso de CFDI',
    'register.facturaSave': 'Enregistrer',
    'register.facturaCancel': 'Annuler',
    'register.card': 'Carte / terminal',
    'register.backspace': 'Retour',
    'register.success': 'SUCCÈS — CLIENT SUIVANT',
    'register.unknownUpc': 'UPC inconnu {upc}',
    'register.unknownUpcOffline': 'UPC inconnu (hors ligne) {upc}',
    'register.scanFirst': 'Scannez un article d’abord',
    'register.decrease': 'Retirer {name}',
    'register.increase': 'Ajouter {name}',
    'register.remove': 'Retirer {name}',
    'register.removeShort': 'Retirer',
    'register.voidTransaction': 'Annuler la transaction',
    'register.voidTitle': 'Annuler cette transaction ?',
    'register.voidBody': 'Un code PIN manager est requis. L’exception est écrite dans le journal hors ligne.',
    'register.voidConfirm': 'Oui, annuler la transaction',
    'register.voidCancel': 'Garder la vente',
    'register.voidPin': 'PIN manager',
    'register.voidPinHint': 'Saisissez le PIN à 4 chiffres',
    'register.voidInvalidPin': 'PIN manager invalide',
    'register.voidValue': 'Panier {amount}',
    'register.signIn': 'Connexion',
    'register.offline': 'Hors ligne',
    'register.online': 'Prêt',
    'register.placeHint': 'Ouvert à {place}',
    'register.currencyMismatch': 'WMS {wms} · local {place}',
    'register.itemsCount': '{n} articles',
    'register.tendered': 'Reçu',
    'register.changeDue': 'Monnaie',
    'register.qtyEach': '{qty} × {price}',
    'register.lane': 'Caisse',
    'locked.title': 'Le POS n’est pas activé',
    'locked.body':
      'Cet espace ou cet abonnement n’inclut pas le POS. Demandez à un propriétaire d’activer le module dans le WMS avant d’encaisser.',
    'locked.signin': 'Se connecter avec un autre compte',
  },
} as const;

export type PosMessageKey = keyof (typeof DICTIONARY)['en'];

export function normalizePosLanguage(raw?: string | null): PosLanguage {
  if (!raw) return 'en';
  const token = raw.trim().toLowerCase().replace('_', '-');
  if (token.startsWith('es')) return 'es';
  if (token.startsWith('fr')) return 'fr';
  return 'en';
}

export function translate(
  language: PosLanguage,
  key: PosMessageKey,
  vars?: Record<string, string>,
): string {
  const table = DICTIONARY[language] ?? DICTIONARY.en;
  let text: string = table[key] ?? DICTIONARY.en[key] ?? key;
  if (vars) {
    for (const [name, value] of Object.entries(vars)) {
      text = text.replaceAll(`{${name}}`, value);
    }
  }
  return text;
}
